//
//  CheerUpMessages.swift
//  AnxietySmasher
//
//  Created by 김유림 on 2/18/25.
//

import Foundation

struct CheerUpMessages {
    
    static let messages: [String] = [
        "Hi, how are you doing?\nHope you’re doing well:)",
        "I hope\nbeautiful things happen to you today.",
        "You're stronger than you think,\nkeep going!",
        "You’ve got this,\ndon’t give up!",
        "Take a deep breath,\neverything will be okay.",
        "Believe in yourself—you’re amazing!",
        "You’re doing great, even if it doesn’t feel like it.",
        "Every step forward is progress, no matter how small.",
        "You are enough, just as you are.",
        "Sending you good vibes and positive energy.",
        "Take it one step at a time, you’re making progress.",
        "You're capable of handling whatever comes your way.",
        "Keep your head up, the best is yet to come.",
        "You deserve all the happiness that’s on its way to you.",
        "Don’t be too hard on yourself, you’re doing the best you can.",
        "Trust the process—you're on the right path.",
        "You’ve got this! Keep pushing through.",
        "You are worthy of all the love and kindness in the world.",
        "Stay strong, things will get better.",
        "Your effort is enough, and so are you.",
        "Don’t forget to take care of yourself. You matter.",
        "You are amazing and capable of achieving incredible things.",
        "This too shall pass, and you’ll come out stronger.",
        "You are loved, and you’re not alone in this.",
        "Keep going, you’re doing more than you know.",
        "You are enough, don’t let anyone tell you otherwise.",
        "Keep believing in yourself, you’re closer than you think.",
        "You are stronger than your struggles.",
        "Take it easy on yourself, progress is still progress.",
        "Stay positive, even when things are tough.",
        "You’re doing your best, and that’s all that matters.",
        "Don’t forget to smile today—you’re doing great!",
        "You are making a difference, even if you can't see it yet.",
        "Take a deep breath, you’ve got everything you need to succeed.",
        "You’re doing amazing, keep up the great work!",
        "One day at a time, you’re on your way.",
        "You’re not alone in this—take it one step at a time.",
        "You’re capable of overcoming anything, I believe in you.",
        "Every small effort counts—keep moving forward.",
        "You are your best friend—be kind to yourself.",
        "No matter how tough it gets, you are tougher.",
        "Your courage is inspiring—keep pushing forward.",
        "It’s okay to take a break—you deserve it.",
        "Your journey is unique and beautiful—keep going.",
        "You have the strength to face any challenge that comes your way.",
        "You are doing your best, and that’s more than enough.",
        "There’s no limit to what you can do—believe in yourself!",
        "You’re capable of great things, keep striving for your dreams.",
        "Remember, it's okay to ask for help when you need it.",
        "You are more resilient than you realize.",
        "The world is lucky to have you in it.",
        "Your potential is limitless—never stop chasing your dreams.",
        "Keep moving forward, one step at a time.",
        "Even on tough days, you are doing your best.",
        "You’re a warrior—don’t forget that!",
        "The best is yet to come, keep believing in yourself."
    ]
    
}
